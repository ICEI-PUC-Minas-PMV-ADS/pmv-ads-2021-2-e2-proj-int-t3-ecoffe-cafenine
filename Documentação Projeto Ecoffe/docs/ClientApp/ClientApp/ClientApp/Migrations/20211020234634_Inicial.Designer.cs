// <auto-generated />
using System;
using ClientApp.Models;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

namespace ClientApp.Migrations
{
    [DbContext(typeof(ApplicationDbContext))]
    [Migration("20211020234634_Inicial")]
    partial class Inicial
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("Relational:MaxIdentifierLength", 128)
                .HasAnnotation("ProductVersion", "5.0.11")
                .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

            modelBuilder.Entity("Client.App.Models.Produto", b =>
                {
                    b.Property<int>("Id_Produto")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Nm_Produto")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<float>("Nr_Altura")
                        .HasColumnType("real");

                    b.Property<float>("Nr_Largura")
                        .HasColumnType("real");

                    b.Property<float>("Nr_Peso_Liquido")
                        .HasColumnType("real");

                    b.Property<int?>("PedidoId_Pedido")
                        .HasColumnType("int");

                    b.Property<string>("Tx_Informacao_Comercial")
                        .HasColumnType("nvarchar(max)");

                    b.Property<float>("Vl_Preco_Produto")
                        .HasColumnType("real");

                    b.HasKey("Id_Produto");

                    b.HasIndex("PedidoId_Pedido");

                    b.ToTable("Produto");
                });

            modelBuilder.Entity("ClientApp.Models.Cartao", b =>
                {
                    b.Property<int>("Id_Catao")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<DateTime>("Dt_Validade")
                        .HasColumnType("datetime2");

                    b.Property<string>("Nm_Cartao")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("Nr_Cartao")
                        .HasColumnType("int");

                    b.Property<string>("Tx_Bandeira")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id_Catao");

                    b.ToTable("Cartao");
                });

            modelBuilder.Entity("ClientApp.Models.Cupom", b =>
                {
                    b.Property<int>("Id_Cupom")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Tx_Cupom")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<float>("Vl_Cupom")
                        .HasColumnType("real");

                    b.HasKey("Id_Cupom");

                    b.ToTable("Cupom");
                });

            modelBuilder.Entity("ClientApp.Models.Endereco", b =>
                {
                    b.Property<int>("Id_Endereco")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Nr_Cep")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Nr_Endereco")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Tx_Endereco")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id_Endereco");

                    b.ToTable("Endereco");
                });

            modelBuilder.Entity("ClientApp.Models.EtapaVenda", b =>
                {
                    b.Property<int>("Id_Etapa_Venda")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Tx_Nome_Usuario")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id_Etapa_Venda");

                    b.ToTable("EtapaVenda");
                });

            modelBuilder.Entity("ClientApp.Models.Frete", b =>
                {
                    b.Property<int>("Id_Frete")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int>("Id_Status_Frete")
                        .HasColumnType("int");

                    b.Property<string>("Nm_Status_Frete")
                        .HasColumnType("nvarchar(max)");

                    b.Property<float>("Vl_Frete")
                        .HasColumnType("real");

                    b.HasKey("Id_Frete");

                    b.ToTable("Frete");
                });

            modelBuilder.Entity("ClientApp.Models.NotaFiscal", b =>
                {
                    b.Property<int>("Id_NotaFiscal")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("NR_NOTA_FISCAL")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("XML")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id_NotaFiscal");

                    b.ToTable("NotaFiscal");
                });

            modelBuilder.Entity("ClientApp.Models.Pedido", b =>
                {
                    b.Property<int>("Id_Pedido")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int?>("CupomId_Cupom")
                        .HasColumnType("int");

                    b.Property<DateTime>("DT_Pedido")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("DataConfirmacao")
                        .HasColumnType("datetime2");

                    b.Property<int?>("FreteId_Frete")
                        .HasColumnType("int");

                    b.Property<int?>("Id_Cupom")
                        .HasColumnType("int");

                    b.Property<int>("Id_Frete")
                        .HasColumnType("int");

                    b.Property<int>("Id_NotaFiscal")
                        .HasColumnType("int");

                    b.Property<int>("Id_Usuario")
                        .HasColumnType("int");

                    b.Property<int?>("NotaFiscalId_NotaFiscal")
                        .HasColumnType("int");

                    b.Property<decimal>("PesoTotal")
                        .HasColumnType("decimal(18,2)");

                    b.Property<int?>("UsuarioId_Usuario")
                        .HasColumnType("int");

                    b.HasKey("Id_Pedido");

                    b.HasIndex("CupomId_Cupom");

                    b.HasIndex("FreteId_Frete");

                    b.HasIndex("NotaFiscalId_NotaFiscal");

                    b.HasIndex("UsuarioId_Usuario");

                    b.ToTable("Pedido");
                });

            modelBuilder.Entity("ClientApp.Models.Seguranca", b =>
                {
                    b.Property<int>("Id_Cod_Seguranca")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<string>("Cod_Seguranca")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id_Cod_Seguranca");

                    b.ToTable("Seguranca");
                });

            modelBuilder.Entity("ClientApp.Models.Usuario", b =>
                {
                    b.Property<int>("Id_Usuario")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasAnnotation("SqlServer:ValueGenerationStrategy", SqlServerValueGenerationStrategy.IdentityColumn);

                    b.Property<int?>("CartaoId_Catao")
                        .HasColumnType("int");

                    b.Property<int>("Id_Cartao")
                        .HasColumnType("int");

                    b.Property<int>("Id_Endereco")
                        .HasColumnType("int");

                    b.Property<bool>("In_Admin")
                        .HasColumnType("bit");

                    b.Property<bool>("In_Ativo")
                        .HasColumnType("bit");

                    b.Property<string>("Nm_Usuario")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("Nr_telefone")
                        .HasColumnType("int");

                    b.Property<int>("Tx_Cnpj")
                        .HasColumnType("int");

                    b.Property<string>("Tx_Cpf")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Tx_Senha")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("tx_Email")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id_Usuario");

                    b.HasIndex("CartaoId_Catao");

                    b.ToTable("Usuario");
                });

            modelBuilder.Entity("Client.App.Models.Produto", b =>
                {
                    b.HasOne("ClientApp.Models.Pedido", null)
                        .WithMany("Produtos")
                        .HasForeignKey("PedidoId_Pedido");
                });

            modelBuilder.Entity("ClientApp.Models.Pedido", b =>
                {
                    b.HasOne("ClientApp.Models.Cupom", "Cupom")
                        .WithMany()
                        .HasForeignKey("CupomId_Cupom");

                    b.HasOne("ClientApp.Models.Frete", "Frete")
                        .WithMany()
                        .HasForeignKey("FreteId_Frete");

                    b.HasOne("ClientApp.Models.NotaFiscal", "NotaFiscal")
                        .WithMany()
                        .HasForeignKey("NotaFiscalId_NotaFiscal");

                    b.HasOne("ClientApp.Models.Usuario", "Usuario")
                        .WithMany()
                        .HasForeignKey("UsuarioId_Usuario");

                    b.Navigation("Cupom");

                    b.Navigation("Frete");

                    b.Navigation("NotaFiscal");

                    b.Navigation("Usuario");
                });

            modelBuilder.Entity("ClientApp.Models.Usuario", b =>
                {
                    b.HasOne("ClientApp.Models.Cartao", "Cartao")
                        .WithMany()
                        .HasForeignKey("CartaoId_Catao");

                    b.Navigation("Cartao");
                });

            modelBuilder.Entity("ClientApp.Models.Pedido", b =>
                {
                    b.Navigation("Produtos");
                });
#pragma warning restore 612, 618
        }
    }
}
